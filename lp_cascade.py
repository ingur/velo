...
# gpx_path = "gps/AMSTERDAM_OSV.gpx"
# exc_path = "Bijlage 5 Assetoverzicht OVL te inspecteren lichtpunten.xlsx"
...
def cascader(vid_path, img_path, cas_path, out_path, save_video=False, out_video_name="output"):
    """
    Draws bounding boxes around light post from a video file
    and outputs them as individual frames.
    vid_path        : path to video file to draw on
    img_path        : destination path for the individual frames
    cas_path        : path to cascade xml file
    out_path        : destination path for the drawn frames
    save_video      : whether to save the frames as a video file
    out_video_name  : name of the output video
    """
    ...
    for f in os.listdir(img_path):
        ...
        # continue if no lampposts are detected in the frame
        if len(lps) == 0:
            continue
